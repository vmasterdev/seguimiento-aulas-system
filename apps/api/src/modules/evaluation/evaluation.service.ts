...
import { isCourseExcludedFromReview, isVacioWithStudents } from '../common/review-eligibility.util';
import { areTemplatesReplicationCompatible } from '../common/template-family.util';

type ChecklistPrimitive = string | number | boolean | null;
type Checklist = Record<string, ChecklistPrimitive>;
const VACIO_WITH_STUDENTS_MARKER = 'vacio_con_estudiantes';
...
const EvaluationManualOverrideSchema = z.object({
  courseId: z.string().trim().min(1),
  phase: z.enum(['ALISTAMIENTO', 'EJECUCION']),
...
function findMissingChecklistItems(input: {
  ...
  if (input.phase === 'ALISTAMIENTO') {
    ...
    if (input.template === 'INNOVAME' || input.template === 'D4') {
      ...
      const hasPresentationKey = Object.prototype.hasOwnProperty.call(input.checklist, 'presentacion');
      const hasFpKey = Object.prototype.hasOwnProperty.call(input.checklist, 'fp');
      const hasFnKey = Object.prototype.hasOwnProperty.call(input.checklist, 'fn');
      if (hasPresentationKey) {
        if (!isChecked('presentacion')) missing.push('Presentacion');
      } else {
        if (!hasFpKey || !isChecked('fp')) missing.push('Foro presentacion');
        if (!hasFnKey || !isChecked('fn')) missing.push('Foro novedades');
      }
    ...
    if (input.template === 'CRIBA') {
      ...
      if (!(isChecked('asistencia') || isChecked('asis'))) missing.push('Asistencia');
      if (!(isChecked('criba_aa') || isChecked('actualizacion_actividades') || isChecked('aa'))) {
        missing.push('CRIBA AA - Actualizacion actividades');
      }
      return missing;
    }
...
function buildObservations(input: {
  ...
  if (!missing.length) {
    ...
    if (input.phase === 'EJECUCION' && input.executionPolicy === 'AUTO_PASS') {
      return 'Cumple automaticamente con ejecucion por politica del periodo.';
    }
    if (input.phase === 'ALISTAMIENTO') {
...
export class EvaluationService {
  ...
  private async replicateFromSelectedCourse(input: {
    ...
    const sourceProgramCode =
      resolveProgramValue({
        ...
        teacherLinked: !!sourceCourse?.teacherId,
    ...
    for (const group of sampleGroups) {
      ...
      for (const candidate of candidates) {
        ...
        if (!areTemplatesReplicationCompatible(candidateTemplate, group.template)) continue;
  ...
  async score(rawPayload: unknown) {
    ...
    const vacioWithStudents =
      payload.phase === 'ALISTAMIENTO' &&
      isVacioWithStudents({
        rawJson: course.rawJson,
        template,
      });
    if (vacioWithStudents && Object.keys(checklist).length === 0) {
      checklist[VACIO_WITH_STUDENTS_MARKER] = true;
    }
  ...
  async manualOverride(rawPayload: unknown) {
    const payload = parseWithSchema(
      EvaluationManualOverrideSchema,
      rawPayload,
      'evaluation manual override request',
    );
    ...
    const checklist = toChecklist(existing?.checklist) as Prisma.InputJsonObject;
    const observations = payload.observations?.trim() || existing?.observations?.trim() || '';
    ...
    const replication = payload.replicateToGroup
      ? await this.replicateFromSelectedCourse({
          selectedCourseId: course.id,
          periodId: course.periodId,
          teacherId: course.teacherId,
  ...
  async recalculate(rawPayload: unknown) {
    ...
    for (const course of courses) {
      ...
      for (const phase of phases) {
        ...
        if (vacioWithStudents) {
          checklist[VACIO_WITH_STUDENTS_MARKER] = true;
        } else {
          delete checklist[VACIO_WITH_STUDENTS_MARKER];
        }
        const checklistJson = checklist as Prisma.InputJsonObject;
  ...
  async replicateSampled(rawPayload: unknown) {
    ...
    for (const group of groups) {
      ...
      if (
        isCourseExcludedFromReview({
          rawJson: selectedCourse.rawJson,
          template: selectedCourse.moodleCheck?.detectedTemplate ?? selectedCourse.templateDeclared ?? 'UNKNOWN',
          moodleCheck: selectedCourse.moodleCheck,
        })
      ) {
        skippedGroups += 1;
        continue;
