...
function defaultGlobalPeriodCodes(periods: PeriodOption[]): string[] {
  return periods
    .map((item) => item.code)
...
export function OutboxEmailPanel({ apiBase }: OutboxEmailPanelProps) {
  ...
  const [selectedPeriodCodes, setSelectedPeriodCodes] = useState<string[]>(defaultGlobalPeriodCodes(FALLBACK_PERIODS));
  ...
  useEffect(() => {
    ...
    async function loadOptions() {
      try {
        const response = await fetchJson<OptionsResponse>(`${apiBase}/outbox/options?yearPrefix=2026`);
        if (!active) return;
        setOptions(response);
        const defaultPeriods = defaultGlobalPeriodCodes(response.periods);
        setSelectedPeriodCodes((current) => (current.length ? current : defaultPeriods));
        setSinglePeriodCode((current) => current || defaultPeriods[0] || response.periods[0]?.code || '202615');
      } catch (error) {
        if (!active) return;
  ...
  function buildSendPayload(dryRun: boolean) {
    ...
    if (lastGeneratedIds.length) payload.ids = lastGeneratedIds;
  ...
  async function runSend(dryRun: boolean) {
    const data = await fetchJson<OperationResponse>(`${apiBase}/outbox/send`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(buildSendPayload(dryRun)),
    });
    setResult(data);
    if (dryRun) {
  ...
  return (
    <article className="panel">
      ...
      <div className="actions" style={{ marginTop: 6 }}>
        Coordinadores y reporte global permiten seleccionar varios periodos 2026 y varios momentos al mismo tiempo.
      </div>
      <div className="actions" style={{ marginTop: 6 }}>
        Seleccion predeterminada consolidada: <span className="code">MD1 + 1</span> y todos los periodos 2026 disponibles, excepto sufijos <span className="code">80</span> y <span className="code">85</span>.
      </div>
      ...
      {!multiPeriodMode ? (
        ...
        <div style={{ marginTop: 12 }}>
          <div className="muted" style={{ marginBottom: 8 }}>
            Periodos 2026 incluidos en la campana ({effectivePeriodCodes.length} seleccionados)
          </div>
